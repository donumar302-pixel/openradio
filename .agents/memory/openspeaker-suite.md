...
cdn.ai33.pro (OpenSpeaker result CDN) times out from many customer networks. Never hand the browser a raw provider URL: every player/download must go through GET /api/os/tasks/:id/file?u=<url> (streams via server; validates the URL appears in that task's stored output — no open proxy; dl=1&name= for attachments). Frontend: os-api.ts taskFileUrl() is applied inside taskAudioUrl/taskSongs/taskImageUrls/taskDownloads — new result fields must be routed through it too.
