...
- **Voice id format differs by endpoint and provider**: `/v3/...` endpoints accept the app's prefixed IDs. Legacy `/v1/task/...` forwards ElevenLabs-backed IDs raw: strip `elevenlabs_` for Voice Changer and Dubbing, and also strip `clone_` for Voice Changer. Provider docs advertise `clone_`, but production processing rejected it as unavailable while the raw clone ID works. Keep prefixed IDs for storage, validation, and ownership; normalize only at submission.
