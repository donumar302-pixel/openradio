...
- **Voice Changer has separate library/clone contracts**: library voices use `/v1/task/voice-changer` with multipart field `file` and raw ElevenLabs IDs. Clones fail there in both raw and `clone_` forms. Clones must use `/v3/text-to-speech/voice-changer`, field `input`, and the prefixed `clone_` ID. Keep prefixed IDs for storage, validation, and ownership.
- **Clone Voice Changer creation is slow and ambiguous on disconnect**: v3 create calls can take over 60s, so return the local task before provider acceptance and attach the provider ID asynchronously. Transport/no-ID outcomes must remain non-refunded and webhook-recoverable; cancellation/deletion is unsafe until acceptance is known.
- **Stored clones can outlive the upstream voice**: the local clone table is an ownership allowlist, not proof the voice still exists. Reconcile owned clone IDs against `/v3/voices?provider=clone` before listing or generating so stale IDs do not spend minutes ending in `voice_not_found`. **Why:** this happened with production clones after the upstream catalog no longer contained their stored IDs. **How to apply:** keep ownership checks local, but require current upstream availability for clone-backed generation.

# Guardrail decisions (keep these invariants when touching /api/os)

- Reserve → local task insert → provider call is two-phase: provider-call failures settle the task as error and refund atomically; after provider acceptance, tracking failures must not refund. **Why:** refunding after acceptance lets users recover credits for work the provider still bills.
...
## Provider CDN is unreachable for end users — always proxy result files
cdn.ai33.pro (OpenSpeaker result CDN) times out from many customer networks. Never hand the browser a raw provider URL — all task result files (audio/srt/images/songs) and any NEW result fields must stream through the server-side, ownership-checked task-file proxy, with exact-match URL provenance against the task's stored output (SSRF boundary) and Range passthrough for audio seeking.
