...
- **Voice Changer has separate library/clone implementations**: library voices use `/v1/task/voice-changer` with multipart field `file` and raw ElevenLabs IDs. The provider's clone-aware `/v3/text-to-speech/voice-changer` accepts tasks but its worker repeatedly ends them as `task_execution_orphaned`; do not use or retry it. Clone conversion is implemented as speech-to-text followed by `/v3/text-to-speech` with the owned prefixed `clone_` ID. This produces audio reliably but re-synthesizes the transcript rather than preserving source prosody.
