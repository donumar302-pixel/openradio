...
import { ShoppingCart, RefreshCw, Check, X, Search, Clock, CheckCircle, XCircle, Trash2 } from "lucide-react";
...
export default function AdminOrders() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => fetch(`/api/admin/orders/${id}`, { method: "DELETE" }),
          ...
          filtered.map((o: any) => {
            ...
            return (
              <div key={o.id} className={cn(
                ...
                isActioning ? "bg-white/[0.04]" : "hover:bg-white/[0.02]"
              )}>
                <div className="grid grid-cols-[1fr_100px_100px_auto] px-5 py-3.5 items-center">
                  {/* User */}
                  <div className="min-w-0 pr-4">
                    <p className="text-[13px] font-semibold text-white truncate">{o.userName}</p>
                    <p className="text-[11px] text-white/30 truncate">{o.userEmail}</p>
                    {o.notes && <p className="text-[11px] text-white/20 truncate mt-0.5 italic">"{o.notes}"</p>}
                    <p className="text-[10px] text-white/15 mt-0.5">
                      {new Date(o.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 justify-end">
                    {o.status === "pending" && !isActioning && (
                      <>
                        <button
                          onClick={() => handleAction(o.id, "approve")}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-green-500/15 text-green-400 text-[11px] font-bold hover:bg-green-500/25 transition-colors"
                        >
                          <Check size={10} /> Approve
                        </button>
                        <button
                          onClick={() => handleAction(o.id, "reject")}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-red-500/10 text-red-400 text-[11px] font-bold hover:bg-red-500/20 transition-colors"
                        >
                          <X size={10} /> Reject
                        </button>
                      </>
                ...
                {isActioning && (
                  <div className={cn(
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-[#0f1117] border border-white/10 rounded-lg px-3 py-2 text-[12px] text-white placeholder:text-white/20 focus:outline-none focus:border-primary/50 resize-none mb-3"
                    />
