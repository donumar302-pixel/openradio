  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-8">
      ...
      <div className="space-y-5 bg-white rounded-2xl border border-border p-6 shadow-sm">
        ...
        <OsCostEstimate estimate={file ? estimateSttCost(file.size) : null} />

        <Button onClick={handleSubmit} disabled={working || !file} className="w-full bg-primary hover:bg-primary/90 font-bold">
          {submitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Starting…</> : <><MessageSquareText className="mr-2 h-4 w-4" />Transcribe</>}
        </Button>
      </div>
